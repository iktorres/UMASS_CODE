...
public class EncryptADFGX {
	private ArrayList<ListPair> alphaIndex   = initializeAlpha();
	private String[][]          polybius     = new String[5][5];
	private String[]            repCharacter = {"A","D","F","G","X"};
	...
	public EncryptADFGX(String key, String message) {
		mutatePolybius();
		setMessage(message);
		setKey(key);
		setEncryptedMessage();
	...
	private void                setMessage(String message) {
		...
		for(int i = 0; i < length; i++) {
			...
			if(isSymbol(temp)) {
				...
				if(!isLetter(temp))
					mess += encodeNumber(Integer.parseInt(temp));
				else
					mess += temp;
			}
		}
